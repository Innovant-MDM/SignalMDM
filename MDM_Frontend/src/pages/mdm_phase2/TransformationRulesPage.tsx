...
export const TransformationRulesPage: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return rules.filter((r) => {
      ...
      const typeOk = typeFilter === 'ALL' || r.transformation_type === typeFilter;
  ...
  const resetCreate = () => {
    setMode('create');
    setForm({ rule_name: '', rule_code: '', transformation_type: 'TRIM', status: 'ACTIVE' });
    setPairs([]);
    setJsonText('{}');
    setJsonError(null);
  };
  ...
  const validate = (): string | null => {
    ...
    if (!/^[A-Z0-9_]+$/.test(form.rule_code.trim().toUpperCase())) {
      return 'Rule code must use only A-Z, 0-9, and underscore';
    }
    if (!form.transformation_type.trim()) return 'Transformation type is required';
  ...
  const onSubmit = async () => {
    ...
    const config_json = kvToObj(pairs);
    setSubmitting(true);
    setError(null);
    try {
      if (mode === 'create') {
        const payload: TransformationRuleCreate = {
          rule_name: form.rule_name.trim(),
          rule_code: form.rule_code.trim().toUpperCase(),
          transformation_type: form.transformation_type.trim().toUpperCase(),
          config_json,
          status: form.status,
        };
        await ruleService.createTransformationRule(payload, activeTenantId ?? undefined);
      } else if (selected) {
        const payload: TransformationRuleUpdate = {
          rule_name: form.rule_name.trim(),
          transformation_type: form.transformation_type.trim().toUpperCase(),
          config_json,
          status: form.status,
        };
        await ruleService.updateTransformationRule(selected.rule_id, payload, activeTenantId ?? undefined);
      }
      await loadRules();
      resetCreate();
      setShowEditor(false);
  ...
  const toggleArchive = async (rule: TransformationRuleRead) => {
    if (!canManage) return;
    const next = rule.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE';
    try {
      await ruleService.patchTransformationRuleStatus(rule.rule_id, next, activeTenantId ?? undefined);
      await loadRules();
    } catch (e) {
        ...
        <button
          ...
          onClick={() =>
            downloadCsv(
              ...
              [
                ['Rule Name', 'Rule Code', 'Type', 'Status', 'Config JSON', 'Updated At'],
                ...filtered.map((r) => [
                  r.rule_name,
                  r.rule_code,
            ...
            <div>
              ...
              {buildDiffRows(historyDetail.old_value, historyDetail.new_value).length === 0 ? (
                ...
                <div className="mdm-rules-table-wrap">
                  ...
                  <input
                    className="mdm-rules-input"
                    placeholder="Search diff path (e.g. config_json.pattern)"
